...
contract PriceOracleImplement is Exponential, V1PriceOracleInterface {
    ...
    uint public constant numBlocksPerPeriod = 2160;// 720; // approximately 1 hour: 60 seconds/minute * 60 minutes/hour * 1 block/15 seconds

    uint public constant maxSwingMantissa = (10 ** 17); // 0.1

    uint public constant defaultPriceExpiredTime = 1080;//
    uint public priceExpiredTime;
    /**
    ...
    constructor(address _poster/*, address addr0, address reader0, address addr1, address reader1*/) public {
        ...
        priceExpiredTime = defaultPriceExpiredTime;
    }

    function setPoster(address newPoster) public{
        require(msg.sender == anchorAdmin,'not authorized' );
        poster = newPoster;
    }
    ...
    event EnableAutoPaused(bool enable);

    event NewPriceExpiredTime(uint oldExpiredTime, uint newExpiredTime);
    /**
    ...
    function _setAutoPause(bool enable) public returns (uint){
        if (msg.sender != anchorAdmin) {
            return failOracle(address(0), OracleError.UNAUTHORIZED, OracleFailureInfo.SET_PAUSED_OWNER_CHECK);
        }

        _autoPause = enable;
        emit EnableAutoPaused(enable);

        return uint(MathError.NO_ERROR);
    }

    function _setExpiredTime(uint expiredTime) public returns (uint){
        if (msg.sender != anchorAdmin) {
            return failOracle(address(0), OracleError.UNAUTHORIZED, OracleFailureInfo.SET_PAUSED_OWNER_CHECK);
        }

        uint old = priceExpiredTime;
        priceExpiredTime = expiredTime;
        emit NewPriceExpiredTime(old,expiredTime);
    ...
    function assetPrices(address asset) public view returns (uint) {
        ...
        if (paused || ((block.number - _lastUpdateTime > priceExpiredTime) && _autoPause)) {
            return 0;
        } else {
            if (address(readers[asset]) != address(0)) {
                ...
                if (foundValue) {
                    (MathError error, Exp memory invertedVal) = getExp(mantissaOne, uint256(readValue));
